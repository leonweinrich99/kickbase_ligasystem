...
def get_player_market_value(token, competition_id, player_id, last_mv_values):
    ...
    return [
        {"mv": value, "date": (epoch + timedelta(days=days)).date().isoformat()}
...
def get_player_performance(token, competition_id, player_id, last_pfm_values, player_team):
    ...
    all_ph = [m for item in data["it"] for m in item["ph"]]
    ...
    for m in performance_values:
        ...
        t1g, t2g = m.get("t1g"), m.get("t2g")
        won = None
        if t1g is not None and t2g is not None:
            if player_team == t1:
                won = 1 if t1g > t2g else (0 if t1g < t2g else None)
            elif player_team == t2:
                won = 1 if t2g > t1g else (0 if t2g < t1g else None)
