...
const Header = ({ matchday, participants }) => (
  <div className="flex justify-between items-center mb-8 border-b border-[#2a2e37] pb-6">
    <div className="flex items-center gap-6">
      <div className="w-16 h-16 bg-[#1a1d24] border border-[#2a2e37] rounded-lg flex relative p-2 shadow-inner">
        <div className="w-full flex flex-col justify-end items-center gap-0.5">
          <div className="w-4 h-2 bg-[#ff5c3e] rounded-sm transform origin-bottom-left rotate-y-3 skew-y-6"></div>
          <div className="w-6 h-2 bg-[#4ba6ff] rounded-sm transform origin-bottom-left rotate-y-3 skew-y-6"></div>
          <div className="w-8 h-2 bg-[#22c55e] rounded-sm transform origin-bottom-left rotate-y-3 skew-y-6"></div>
          <div className="w-10 h-2 bg-[#10b981] rounded-sm transform origin-bottom-left rotate-y-3 skew-y-6"></div>
        </div>
      </div>
      <div>
        <div className="text-[11px] font-bold tracking-wider text-[#ff5c3e] mb-1">SAISON 26/27</div>
        <h1 className="text-3xl font-black tracking-tight uppercase">Qualifikationsrunde</h1>
      </div>
    </div>
    <div className="flex gap-4">
      <div className="bg-[#1a1d24] border border-[#2a2e37] rounded-xl flex items-center shadow-lg font-semibold overflow-hidden">
        <button className="px-4 py-3 text-[#8b92a5] hover:text-white transition-colors bg-[#181a20]">&lsaquo;</button>
        <span className="px-8 text-sm text-gray-200">Spieltag {matchday}</span>
        <button className="px-4 py-3 text-[#8b92a5] hover:text-white transition-colors bg-[#181a20]">&rsaquo;</button>
      </div>
      <div className="bg-[#1a1d24] border border-[#2a2e37] rounded-xl px-5 py-2 shadow-lg flex flex-col justify-center items-end">
        <span className="text-[10px] font-bold text-[#8b92a5] tracking-widest leading-tight">TEILNEHMER</span>
        <span className="text-base font-bold text-gray-200 leading-tight block -mt-1">{participants}</span>
      </div>
      <div className="bg-[#1a1d24] border border-[#2a2e37] rounded-xl w-14 flex justify-center items-center text-[#8b92a5]">
...
const LeagueColumn = ({ league }) => (
  <div className="flex-1 min-w-[320px] px-2.5">
    <div className="flex items-center mb-4">
      <div className="w-1 h-5 mr-3 rounded-full" style={{ backgroundColor: league.color }}></div>
      <h2 className="text-lg font-black tracking-wider uppercase text-gray-200">{league.name}</h2>
    </div>
...
function App() {
  ...
  return (
    <div className="min-h-screen bg-[#0f1115] p-10 font-sans select-none">
      ...
      <div className="mt-20 text-center text-[#555] text-xs">
        *The data above is retrieved automatically via backend Node-Cron.
      </div>
    </div>
  );
}
